...
5. **Design evidence.** Specify exact calculations, numerical experiments, controls,
   ...
   them findings.
6. **Implement and falsify.** Put model logic in `src/study/`, add tests, run the claim
   evaluators, and retain counterexamples or failed checks that alter interpretation.
...
8. **Approve and package release.** Remove placeholders, audit claims and citations,
   approve the release gate, and run `paperkit release`.
